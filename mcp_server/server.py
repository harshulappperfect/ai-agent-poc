...
def get_financial_data(org_id: str, month: str | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        return [{"error": f"Database query failed: {e}. Please check if the Docker container 'finance-postgres' is running on port 5432."}]
...
def get_org_summary(org_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": f"Database query failed: {e}. Please check if the Docker container 'finance-postgres' is running on port 5432.",
            "org_id": clean_org,
        }
    ...
    total_forecast = float(row.get("total_forecast", 0.0))
    total_actual = float(row.get("total_actual", 0.0))
    variance = total_actual - total_forecast
